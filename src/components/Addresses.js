...
const mapDispatchToProps = (dispatch, ownProps) => {
    return {
        handleGenerateWalletAddressButton: (asset) => {
            dispatch(generateWalletAddress(asset.name))
        }
    }
}

const Addresses = ({address, mnemonic, handleGenerateWalletAddressButton}) => {
    const getAddresses = (asset) =>{
        if (asset in address){
    ...
    if (mnemonic) {
        ...
        return(<h1>Generate Seed using button above</h1>)
    }
}

const AddressContainer = connect(mapStateToProps,mapDispatchToProps)(Addresses)

export default AddressContainer

const assetData = {
    //TECH DEBT: once lib returns this data, move this to state tree
    "BTC": {
        "name": "Bitcoin",
        "symbol": "BTC",
